  ...
  return (
    <>
      {categoryBody.map((offerItem) => (
        <div
          key={offerItem.id}
          className=" h-[28rem]  my-6  md:mx-2 sm:h-[28rem] md:h-[34rem] 2xl:h-[42rem]  bg-white shadow-lg shadow-gray-400 animate-fadeIn rounded-xl   transition-transform duration-200 ease-in-out hover:scale-[1.02]    2xl:w-[30rem]  "
        >
          <Link className="" href={`/offer/${offerItem.slug} `}>
            <div className="relative h-[28rem] w-[20rem]   sm:w-[20rem] md:w-[17rem] md:h-[34rem] lg:min-w-[20rem] xl:w-[25rem] 2xl:h-[42rem] 2xl:w-[30rem] ">
              <Image
                src={`/${offerItem.image}`}
                alt={offerItem.title}
                fill
              ...
              <div className="absolute bottom-0 left-0 bg-black bg-opacity-35   rounded w-full  ">
                <div className=" flex flex-col justify-center items-center animate-fadeIn rounded   px-1  w-full  ">
                  ...
                  <p className=" text-center text-base  xl:text-md 2xl:text-xl font-semibold indent-4 px-8  sm:px-2 h-32 sm:h-32 md:h-28 lg:h-24 2xl:h-24 text-neutral-200 py-2 ">
